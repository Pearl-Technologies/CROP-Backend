...
} = require("./businessNotification/accountNotification")
...
const createOrUpdateCropRules = async (req, res) => {
  ...
  try {
    ...
    await createProgramChangesNotification("earnPoints", businessId)
...
const createOrUpdateExtendBonusCrops = async (req, res) => {
  ...
  try {
    ...
    await createProgramChangesNotification("bonusPoints", businessId)
...
const createOrUpdateSlashRedeemptionCrops = async (req, res) => {
  const businessId = req.user.user.id
  try {
    const slashRedemptionCropFind = await businessSlashRedemptionCrops.find({
      businessId,
    })
    await createProgramChangesNotification("slashRedeemPoints", businessId)
    if (slashRedemptionCropFind.length <= 0) {
      ...
      console.log("exist running")
      console.log("body", req.body)
      const slashRedemptionCrop =
        await businessSlashRedemptionCrops.findByIdAndUpdate(
...
const getSlashRedemptionCrop = async (req, res) => {
  ...
  console.log({ businessId })
  try {
    const slashRedemptionCrop = await businessSlashRedemptionCrops.find({
      businessId,
    })
...
const createOrUpdateHappyHours = async (req, res) => {
  ...
  try {
    ...
    await createProgramChangesNotification("happyHours", businessId)
